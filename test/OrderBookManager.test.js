...
contract("OrderBookManager", (accounts) => {
    ...
    it("should create an order", async () => {
        ...
        const order = await orderBook.getOrderFromId(orderType, receipt.logs[0].args.orderId);
    ...
    it("should cancel an order", async () => {
        ...
        // convert to number as returned as string
    ...
    it("should get all orders for a market with filters", async () => {
        ...
        const result = await orderBookManager.getAllOrdersForAMarket(marketId, params);
    ...
    it("should match 1 sell limit order to 1 buy limit order", async () => {
        ...
        const initialSupply = new BN(web3.utils.toWei("1000", "ether"));
        await tokenManager.issueToken("Ether", "ETH", new BN(web3.utils.toWei("1000", "ether")), { from: deployer });
        await tokenManager.issueToken("USD", "USD", new BN(web3.utils.toWei("1000", "ether")), { from: deployer });
        ...
        await ethToken.transfer(user1, new BN(web3.utils.toWei("500", "ether")), { from: deployer });
        await usdToken.transfer(user2, new BN(web3.utils.toWei("600", "ether")), { from: deployer });

        // Approve tokens from user1 and user2 end
        await ethToken.approve(tokenManager.address, new BN(web3.utils.toWei("500", "ether")), { from: user1 });
        await usdToken.approve(tokenManager.address, new BN(web3.utils.toWei("600", "ether")), { from: user2 });

        // User deposits token to tokenManager
        await tokenManager.deposit("ETH", new BN(web3.utils.toWei("500", "ether")), { from: user1 });
        await tokenManager.deposit("USD", new BN(web3.utils.toWei("600", "ether")), { from: user2 });
        ...
        const price1 = new BN(web3.utils.toWei("1", "ether"));
        const amount2 = new BN(web3.utils.toWei("50", "ether"));
        const price2 = new BN(web3.utils.toWei("1", "ether"));
        ...
        expect(order1.remainingAmount).to.be.bignumber.equal(new BN(web3.utils.toWei("50", "ether")));
        expect(order2.remainingAmount).to.be.bignumber.equal(new BN(web3.utils.toWei("0", "ether")));

        expect(Number(order1.status)).to.equal(OrderLibrary.OrderStatus.PartiallyFilled);
    ...
    it("should match 1 sell limit order to 2 buy limit order", async () => {
        ...
        await orderBookManager.createOrder(
            ...
            new BN(web3.utils.toWei("1", "ether")), // Price is 1 USD
        ...
        // Listen for events
        // expectEvent(matchReceipt, "OrderFilledEvent", {
        //     orderId: new BN(1), // 1st buy order is filled (higher price)
        //     orderType: new BN(orderTypeBuy),
        // });
        // expectEvent(matchReceipt, "OrderPartiallyFilledEvent", {
        //     orderId: new BN(2), // 2nd buy order is partially filled 
        //     orderType: new BN(orderTypeBuy),
        // });
        // expectEvent(matchReceipt, "OrderFilledEvent", {
        //     orderId: new BN(1), // sell order is filled
        ...
        expect(order1.remainingAmount).to.be.bignumber.equal(new BN(web3.utils.toWei("0", "ether")));
        expect(order2.remainingAmount).to.be.bignumber.equal(new BN(web3.utils.toWei("30", "ether")));
        expect(order3.remainingAmount).to.be.bignumber.equal(new BN(web3.utils.toWei("0", "ether")));

        expect(Number(order1.status)).to.equal(OrderLibrary.OrderStatus.Filled);
        expect(Number(order2.status)).to.equal(OrderLibrary.OrderStatus.PartiallyFilled);
    ...
    it("should match 1 buy limit order to 2 sell limit order", async () => {
        ...
        // Listen for events
        // expectEvent(matchReceipt, "OrderPartiallyFilledEvent", {
        //     orderId: new BN(1), // 1st sell order is partially filled 
        //     orderType: new BN(orderTypeSell),
        // });
        // expectEvent(matchReceipt, "OrderFilledEvent", {
        //     orderId: new BN(2), // 2nd sell order is filled (lower price)
        //     orderType: new BN(orderTypeSell),
        // });
        // expectEvent(matchReceipt, "OrderFilledEvent", {
        //     orderId: new BN(1), // buy order is filled
        ...
        expect(order1.remainingAmount).to.be.bignumber.equal(new BN(web3.utils.toWei("30", "ether")));
        expect(order2.remainingAmount).to.be.bignumber.equal(new BN(web3.utils.toWei("0", "ether")));
        expect(order3.remainingAmount).to.be.bignumber.equal(new BN(web3.utils.toWei("0", "ether")));
